...
TString legend_text[15] = {"Generated", "At rear surface"};
...
void generate_1Dplot(vector<TH1F*> hist, char const *tag_name="", double xmax=-1.,double xmin=-1.,char const *leg_head="",
  ...
  TCanvas *canvas_n1 = new TCanvas(tag_name, tag_name,1200,800);//600,600,1200,1200);
  ...
  for(int i = 0; i < (int)hist.size(); i++) {
    ...
    hist.at(i)->GetXaxis()->SetTitle(xtitile);
    hist.at(i)->GetXaxis()->SetTitleSize(0.05);
  ...
  for(int i = 0; i < (int)hist.size(); i++) {
    ...
    //setLastBinAsOverFlow(hist.at(i),10);
    ...
    TPaveStats *ptstats1 = new TPaveStats(0.7,0.78,0.955,0.94,"brNDC");
  ...
  latex.DrawLatex(0.25, 0.85, title);
...
void make_1d_single_plots()
{
  ...
  //f= {"out_root_files/Uncorrelated_Edep60_nHit1_1MEvts_1_10_out.root"};
  //f= {"out_root_files/Uncorrelated_Edep60_nHit2_1MEvts_1_10_out.root"};
  //f= {"out_root_files/nOpPho_plots_OpON_4.5MEvts.root"};
  //f= {"out_root_files/OpOFF_Correlated_theta_70_90_5MEvts_correct_DetPos_correct_pol_out.root"};
  f= {"out_root_files/plotting_reco_info_Correlated_OpON_4.5MEvts.root"};
  
  
  //define your histograms to be read from here
  string histname1[100], histname2[100], histname3[100];

  vector<string> variables, diff_title;
  vector<bool> logFlag;

  //-----------------------------------------
  //variables = {"right_nOpPho_gen_mdl_sum", "right_nOpPho_mdl_sum"};
  //variables = {"right_theta_truth"};
  variables = {"right_Edep_reco_cryst", "right_theta_reco", "right_phi_reco", "dPhi_reco_inc"};

  diff_title = {"Reco Edep (MeV)", "#theta (#circ)", "reco #phi (#circ)", "reco #Delta#phi (#circ)"};

  vector<double>xmax = {0.8, 120, 200, 200};
  vector<double>xmin = {0, -200, -200,-200};

  logFlag = {true, false, false, false};
  //-----------------------------------------

  vector<string> filetag;
  filetag={"sim: 4.5M Evts, OpON"};
  ...
  if(logFlag[iVar] == false){
    sprintf(full_path,"%s%s",folder[0].c_str(),variables[iVar].c_str());
    generate_1Dplot(hist_list,full_path,xmax[iVar],xmin[iVar],leg_head,NormFlag,logFlag[iVar],false,true,filetag[0].c_str(),diff_title[iVar].c_str(),rebin[iVar]);
  }

  else {
    sprintf(full_path,"%s%s_logScale",folder[iVar].c_str(),variables[iVar].c_str());
    generate_1Dplot(hist_list,full_path,xmax[iVar],xmin[iVar],leg_head,NormFlag,logFlag[iVar],false,true,filetag[0].c_str(),diff_title[iVar].c_str(),rebin[iVar]);
  }  
  }
}
