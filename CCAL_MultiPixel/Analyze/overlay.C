...
int line_color[15] = {kBlack, kRed, kPink+5, kOrange-3, kCyan+4, kAzure+7, kSpring+8, kPink+10, kGreen + 3,kRed,kGray, kViolet -7 };
...
void overlay()
{
  ...
  for (int ifile =0; ifile < f.size(); ifile++){
    for (int iVar =0; iVar < variables.size(); iVar++){
      ...
      //sprintf(hist_name,"%s", variables[0].c_str());
      cout<<"i_file "<< ifile <<"\t"<< 0 <<"\t"<<root_file->GetName()<< "\t" << hist_name<<endl;
      TH1F* resp = (TH1F*)root_file->Get(hist_name); //reading hist from the TFile
      cout << "XMAX: " << resp->GetXaxis()->GetXmax();
      hist_list.push_back(resp);
    }      //cout << "Integral: " << resp->Integral() << endl;
  }
  ...
  // sprintf(full_path,"%s%s",folder[0].c_str(),variables[0].c_str());
  ...
  logFlag = true;
  sprintf(full_path,"%s%s_logScale",folder[0].c_str(),variables[0].c_str());
  generate_1Dplot(hist_list,full_path,xmax[0],xmin[0],leg_head,NormFlag,logFlag,false,true,filetag[0].c_str(),diff_title[0].c_str(),rebin[0]);
}
