...
TString legend_text[15] = {"Sim: 8M Evts","511keV", "1000keV"};
...
void Make2dPlts(string pathname)
{
  ...
  //f={"single_Pho_out_root_files/Final_allRandomized_100kEvts_out.root"};
  ...
  nHitCat = {"nHit2_AnyDiffDet"};
  //nHitCat = {""};

  char hname_ThetaVsEta[100], hname_ThetaVsEout[100]; 

  
  for(int i=0; i< nHitCat.size(); i++){
    sprintf(hname_ThetaVsEta, "h_theta_eta_%s",nHitCat[i].c_str());
    ...
    variables = {"h_ST1jVsST2j", "Theta_SimAna", "Eta1VsEta2_inc"};
  ...
  //x axis range
  vector<double>xmax = {5, 200, 200, 300, 700, 700, 700};
  vector<double>xmin = {-5, -200, -200, 0, 0, -700, -700};

  vector<double>ymax = {5, 200, 200, 300, 700, 700, 700};
  vector<double>ymin = {-5, -200, -200, 0, 0, -700, -700};
  ...
  for (int iVar =0; iVar < variables.size(); iVar++){
    ...
    diff_Xtitle = {"ST_{1j} (ns)", "#theta_{Ana}(#circ)", "#eta_{1}(#circ)"};
    diff_Ytitle = {"ST_{2j} (ns)", "#theta_{Truth}(#circ)", "#eta_{2}(#circ)"};
    ...
    generate_1Dplot(hist_list,full_path,xmax[iVar],xmin[iVar], ymax[iVar], ymin[iVar], leg_head,false,logFlag,false,true,filetag[i].c_str(),diff_Xtitle[iVar].c_str(), diff_Ytitle[iVar].c_str(), false);
  }
  }
}
