...
void MySteppingAction::UserSteppingAction(const G4Step *step)
{
  ...
  G4int DetId = postStepPoint->GetTouchableHandle()->GetVolume()->GetCopyNo();

  if (DetId != 3) track->SetTrackStatus(fStopAndKill);
  ...
  //if (proc == "compt" && parentId == 0 && postStepPoint->GetTouchableHandle()->GetVolume()->GetName() == "physDetector") {
  if (proc == "compt" && parentId == 0 && postStepPoint->GetTouchableHandle()->GetVolume()->GetName() == "physDetector") {
    G4ThreeVector v1 = preStepPoint->GetMomentumDirection();
    G4ThreeVector v2 = postStepPoint->GetMomentumDirection();

    theta = acos(v1.dot(v2));
    
    fEventAction->nCompt++;
    fEventAction->scat_theta.push_back(theta*180/CLHEP::pi);
