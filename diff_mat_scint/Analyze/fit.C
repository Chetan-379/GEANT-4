...
void fit(){
  string rootfile;
  //rootfile = "BGO_out_root_files/BGO_5cm_ScintON_511keV_Birk0_NoAbs_NoCeren_SefDerEmiss_spline_out.root";
  rootfile = "GAGG_out_root_files/GAGG_5cm_ScintON_511keV_Birk0_NoAbs_spline_out.root";
  ...
  TF1 *func = new TF1("fit",fitf,2,10000,4); 
  //func->SetParameters(0.2, 0.8, -10, -1);
  func->SetParameters(0.608, 0.392, -321, -51.1); 
  ...
  sprintf(chi2ndf, "\\chi^{2}/NDF: %0.2f", func->GetChisquare()/func->GetNDF());

  latex->DrawLatexNDC(0.66,0.75, param1);
  latex->DrawLatexNDC(0.66,0.70, param2);
  latex->DrawLatexNDC(0.66,0.65, param3);
  latex->DrawLatexNDC(0.66,0.60, param4);
  latex->DrawLatexNDC(0.66,0.55, chi2ndf);  
  ...
  gPad->SetLogy();
  //gPad->Update();
  sprintf(canvas_name,"%s.png","Overlay_plots/image");
  canvas_n1->SaveAs(canvas_name);
