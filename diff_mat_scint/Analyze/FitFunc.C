...
vector<double> generate_1Dplot(vector<TH1*> hist, char const *tag_name="", int xmax=-1,int xmin=-1,char const *leg_head="",
			       bool normalize=false, bool log_flag=true, bool DoRebin=false, bool save_canvas=true, char const *title="", const char* xtitile="", const char* ytitile="", int rebin=-1, string PlotsFolder = ""){
  ...
  string SavePath;
  SavePath = PlotsFolder + tag_name + ".png";
  if (save_canvas) canvas_n1->SaveAs(SavePath.c_str());
  //cout << SavePath << endl;

  SavePath = PlotsFolder + tag_name + ".pdf";
  if (save_canvas) canvas_n1->SaveAs(SavePath.c_str());
...
void FitOverlay(vector<TF1*> funcs, vector<TGraphErrors*> graphs, string MatName, string VarName, string PlotsFolder) {
  ...
  string OvrlImgName1, SavePath;
  SavePath = PlotsFolder + "/Scintillation/Fits/Resol_Resp/" + MatName + "/Summary_Plots/";
  if(VarName == "Resol") {    
    OvrlImgName1 = SavePath + MatName + "_resolution_function_overlay";

    string FinalName1;
    FinalName1 = OvrlImgName1 + ".png";
    c1->SaveAs(FinalName1.c_str());    

    FinalName1 = OvrlImgName1 + ".pdf";
    c1->SaveAs(FinalName1.c_str());    
  }
  ...
  string OvrlImgName2;
  if (VarName == "Resol") OvrlImgName2 = SavePath + MatName + "_resolution_points_overlay";
  else if (VarName == "Eff") OvrlImgName2 = SavePath + MatName + "_Efficiency_points_overlay";

  string FinalName2;

  FinalName2 = OvrlImgName2 + ".png";  
  c2->SaveAs(FinalName2.c_str());

  FinalName2 = OvrlImgName2 + ".pdf";
  c2->SaveAs(FinalName2.c_str());
...
void data_plot(string MatName, string txtFilePath, string PlotsFolder) {
  ...
  for (int i=0; i<data_files.size(); i++){
    ...
    MainFolder = "Scintillation/Fits/Resol_Resp";    
    ...
    folder = MainFolder + "/" + MatName + "/depth_" + Width + "/";
    
    ImgName = MatName + "_" + Width + "_" + ImgId;      
    
    //saving the canvas
    string image_name;
    image_name = PlotsFolder + "/" + folder + ImgName + ".png";
    canvas->SaveAs(image_name.c_str());

    image_name = PlotsFolder + "/" + folder + ImgName + ".pdf";
    canvas->SaveAs(image_name.c_str());
  }
}
...
void FitFunc()
{
  ...
  //vector<TString> Folders = {"PbWO4_out_root_files", "BGO_out_root_files", "LYSO_out_root_files", "GAGG_out_root_files", "LaBr3_out_root_files"};
  vector<TString> Folders = {"LaBr3_out_root_files_Trans2.5cm"};
  ...
  for (int iFolder = 0; iFolder < Folders.size(); iFolder++){
    ...
    //string PlotFolder = "Chetan_Thesis_Plots/Trans_20cmX20cm";
    string PlotFolder = "test_plots/";


    string txtPath = PlotFolder + "/Scintillation/Fits/txtFiles/";    
    string MainFolder = "/Scintillation/Fits/ItrFit";

    string Energy, Width, txtFileId, Material;
    int MatIdx, rebin, EnergyIdx;
      ...
      for(int i=0; i< Widths.size(); i++)
	{
      ...
      folder = MainFolder + "/" + Material + "/depth_" + Width;
      ...
      if(Material == "PbWO4"){
	if (Energy == "100keV" || Energy == "150keV") {xmin = 1; xmax = 100;}
	if (Energy == "300keV" || Energy == "450keV") {xmin = 10; xmax = 160;}
	if (Energy == "511keV" || Energy == "600keV") {xmin = 30; xmax = 180;}
	if (Energy == "800keV" || Energy == "1000keV"){xmin = 100; xmax = 250;}	    
      }
      
      //defininig the initial range for fitting
      if(Material == "BGO"){
	if(Width == "5cm") {
	  if (Energy == "100keV" || Energy == "150keV") {xmin = 600; xmax = 1600;}
	...
	if(Width == "10cm") {
	  if (Energy == "100keV") {xmin = 600; xmax = 1000;}
	  if (Energy == "150keV") {xmin = 800; xmax = 1600;}
	  if (Energy == "300keV") {xmin = 1500; xmax = 3000;}
	  if (Energy == "450keV" || Energy == "511keV") {xmin = 3200; xmax = 4600;}
	  if (Energy == "600keV") {xmin = 4000; xmax = 6500;}
	  if (Energy == "800keV") {xmin = 6000 ; xmax = 8000;}
	  if (Energy == "1000keV") {xmin = 7200; xmax=9000;}
	}
	...
	if(Width == "2.5cm") {
	  if (Energy == "100keV") {xmin = 600; xmax = 1000;}
	  if (Energy == "150keV") {xmin = 1000; xmax = 1500;}		
	...
	if(Width == "1cm") {
	  ...
	  if (Energy == "600keV") {xmin = 4500; xmax = 5100;}
	  if (Energy == "800keV") {xmin = 6000 ; xmax = 7000;}
	  if (Energy == "1000keV") {xmin = 7000; xmax = 9000;}
	}
      }       
	...
	if(Width == "1cm") {
      ...
      sprintf(full_path,"%s/%s_%s_%s_%d_%s",folder.c_str(), "nOpticalPhotns_ItrFit", Material.c_str(), Width.c_str(), EnergyIdx, Energy.c_str());
    
      vector<double> params;
      params = generate_1Dplot(hist_list,full_path,xmax,xmin,leg_head,false,false,false,true,filetag.c_str(),xtitle.c_str(),ytitle.c_str(),rebin, PlotFolder);
    ...
    FitOverlay(Fit_array, Graph_array_Resol, Material, "Resol", PlotFolder);
    FitOverlay(Fit_array, Graph_array_Eff, Material, "Eff", PlotFolder);
