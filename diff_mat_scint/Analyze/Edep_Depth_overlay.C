...
void overlay(string)
{
  ...
  for (int iMat =0; iMat<Materials.size(); iMat++){
    for (int iEn =0; iEn<Energies.size(); iEn++){
      ...
      //x axis range
      vector<double>xmax = {0.2, 0.2, 0.4, 0.5, 0.6, 0.7, 0.9, 1.1};
	...
	cout << full_path << endl;	 	      
	generate_1Dplot(hist_list,full_path,xmax[iEn],xmin[EnergyIdx],leg_head,false,true,false,true,filetag[iVar].c_str(),xtitle[iVar].c_str(),rebin[iVar]);
      }
    }
  }
}
