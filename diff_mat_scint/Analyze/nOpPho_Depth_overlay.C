...
void nOpPho_Depth_overlay()
{
  ...
  for (int iMat =0; iMat<Materials.size(); iMat++){
    for (int iEn =0; iEn<Energies.size(); iEn++){
      ...
      //x axis range
      vector<double>xmax_PbWO4 = {100, 100, 160, 160, 180, 180, 250, 250};
      vector<double>xmin_PbWO4 = {1, 1, 10, 10, 30, 30, 100, 100};

      vector<double>xmax_BGO = {950, 1400, 3000, 4500, 5000, 5100, 7000, 8500};
      vector<double>xmin_BGO = {600, 1000, 2000, 3300, 3700, 4500, 6000, 7500};

      vector<double>xmax_LYSO = {3500, 5000, 9600, 14500, 16000, 19000, 25000, 31000};
      vector<double>xmin_LYSO = {2600, 3800, 8200, 12500, 14600, 17000, 23200, 28500};

      vector<double>xmax_GAGG = {3200, 5000, 9500, 14000, 15500, 18000, 24000, 30000};
      vector<double>xmin_GAGG = {2400, 3500, 7500, 11000, 13500, 15500, 21000, 26000};

      vector<double>xmax_LaBr3 = {7000, 10000, 20000, 30000, 34000, 40000, 52000, 65000};
      vector<double>xmin_LaBr3 = {5000, 8000, 16000, 26000, 29000, 32000, 46000, 58000};
      

      vector<vector<double>> xmax = {xmax_PbWO4, xmax_BGO, xmax_LYSO, xmax_GAGG, xmax_LaBr3};
      vector<vector<double>> xmin = {xmin_PbWO4, xmin_BGO, xmin_LYSO, xmin_GAGG, xmin_LaBr3};
	...
	cout << "xMax: " << xmax[iMat][iEn] << endl;
	sprintf(full_path,"%s/%s_%s_%d_%s_%s",folder[iVar].c_str(), "Depths_Overlay", diff_title[iVar].c_str(), iEn, Material.c_str(), Energy.c_str());

	bool logFlag = false;
  
	cout << full_path << endl;	 	      	
	generate_1Dplot(hist_list,full_path,xmax[iMat][iEn],xmin[iMat][iEn],leg_head,false,logFlag,false,true,filetag[iVar].c_str(),xtitle[iVar].c_str(),rebin[iMat]);
      }
    }
  }
}
