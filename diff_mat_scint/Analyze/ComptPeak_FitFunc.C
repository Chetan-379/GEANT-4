...
vector<double> generate_1Dplot(vector<TH1*> hist, char const *tag_name="", int xmax=-1,int xmin=-1,char const *leg_head="",
  ...
  TLegend *legend = new TLegend(0.5, 0.84, 0.80, 0.90);
  ...
  TH1F* hist_uncut, *hist_temp;
  
  for(int i = 0; i < (int)hist.size(); i++) {
    hist_uncut = (TH1F*)hist.at(i)->Clone("hist_uncut");
    if(normalize)   hist.at(i)->Scale(1.0 / hist.at(i)->Integral());
    hist.at(i)->Rebin(rebin);
    hist_temp = (TH1F*)hist.at(i)->Clone("hist_temp");    
    hist_temp->Rebin(10);
    hist_temp->GetXaxis()->SetRangeUser(0,16000);
    hist_temp->GetXaxis()->SetTitle("nOpticalPhotons");
    //hist_temp->GetYaxis()->SetRangeUser(1,200);
    hist.at(i)->GetXaxis()->SetRangeUser(xmin,xmax);
    //hist.at(i)->GetXaxis()->SetRangeUser(100,xmax);
  ...
  for(int i = 0; i < (int)hist.size(); i++) {         
    ...
    latex->DrawLatexNDC(0.50,0.80, param1);
    latex->DrawLatexNDC(0.50,0.75, param2);
    latex->DrawLatexNDC(0.50,0.70, param3);
    latex->DrawLatexNDC(0.50,0.65, chisqrNDF);
...
void FitOverlay(vector<TF1*> funcs, vector<TGraphErrors*> graphs) {
  ...
  //c1->SaveAs("LYSO_resolution_function_overlay.png");
  //c1->SaveAs("BGO_resolution_function_overlay.png");
  //c1->SaveAs("LaBr3_resolution_function_overlay.png");
  c1->Close();
  ...
  c2->SaveAs("LYSO_resolution_points_overlay.png");
}
...
void data_plot() {
  ...
  TSystemDirectory dir("Rep_plots/Scintillation/Fits/txtFiles", "Rep_plots/Scintillation/Fits/txtFiles");
  ...
  while ((obj = next())) {
    ...
    if (fname.EndsWith(".txt")) {
      data_files.push_back("Rep_plots/Scintillation/Fits/txtFiles/" + string(fname.Data()));
    }
  }
  ...
  for (int i=0; i<data_files.size(); i++){
    ...
    if (FileName.Contains("Resolution_vs_Einc")){
      ...
      fit_func->SetTitle("Resolution_BGO; E_{inc}; Resolution");      
      fit_func->Draw("same");
    ...
    MainFolder = "Rep_plots/Scintillation/Fits/Resol_Resp";
    ...
    if(FileName.Contains("LYSO")){
      if (FileName.Contains("2.5cm")) {folder = MainFolder + "/LYSO/2.5cm/"; Width = "2.5cm";}
      if (FileName.Contains("_5cm")) {folder = MainFolder + "/LYSO/5cm/"; Width = "5cm";}
      if (FileName.Contains("10cm")) {folder = MainFolder + "/LYSO/10cm/"; Width = "10cm";}
      
      ImgName = "LYSO_" + Width + ImgId;      
    }
...
void FitFunc(string pathname)
{
  ...
  for(int iFile=0; iFile < f.size(); iFile++){   
    ...
    if(FileId.Contains("Plastic")){
      ...
      if (FileId.Contains("5cm")) {folder = "Rep_plots/" + MainFolder + "/Plastic/5cm"; Width = "5cm";}
	...
	if (Energy == "511keV") {xmin = 13500; xmax = 15500;}
	//if (Energy == "511keV") {xmin = 0; xmax = 15500;}
    ...
    params = generate_1Dplot(hist_list,full_path,xmax,xmin,leg_head,false,true,false,true,filetag.c_str(),xtitle.c_str(),ytitle.c_str(),rebin);

    //storing the parameters in the txt file
    ofstream outFile1("Rep_plots/Scintillation/Fits/txtFiles/Resolution_vs_Einc_" + txtFileId + ".txt", ios::app), outFile2("Rep_plots/Scintillation/Fits/txtFiles/Ngamma_vs_Einc_" + txtFileId + ".txt", ios::app);
    ofstream outFile5("Rep_plots/Scintillation/Fits/txtFiles/Efficiency_vs_Einc_" + txtFileId + ".txt", ios::app);
  ...
  TSystemDirectory dir1("Rep_plots/Scintillation/Fits/txtFiles", "Rep_plots/Scintillation/Fits/txtFiles");
  ...
  while ((obj1 = next1())) {
    ...
    if (fname.Contains("Ngamma") && fname.EndsWith(".txt")) {
      data_files1.push_back("Rep_plots/Scintillation/Fits/txtFiles/" + string(fname.Data()));
    }
  }
  ...
  for (int itxt = 0; itxt < TxtArr.size(); itxt++){
	...
	ifstream ChkFile("Rep_plots/Scintillation/Fits/txtFiles/Ngamma_vs_Einc_" + TxtArr[itxt] + ".txt");
	ofstream outFile3("Rep_plots/Scintillation/Fits/txtFiles/Emeasured_vs_Einc_" + TxtArr[itxt] + ".txt");

	ofstream outFile4("Rep_plots/Scintillation/Fits/txtFiles/Response_vs_Einc_" + TxtArr[itxt] + ".txt");
	while (ChkFile >> x >> y){ 
