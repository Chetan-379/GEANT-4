...
void generate_1Dplot(vector<TH1*> hist, char const *tag_name="", double xmax=-1,int xmin=-1,char const *leg_head="",
...
void make_Edep_plots()
{
  ...
  for (int iFolder = 0; iFolder < Folders.size(); iFolder++){
	...
	if(histId.Contains("Edep") || histId.Contains("Compt")) {
	  ...
	  //generate_1Dplot(hist_list,full_path,xmax[EnergyIdx],xmin[0],leg_head,false,logFlag,Rebin,true,filetag.c_str(),xtitle.c_str(),ytitle.c_str(),rebin[0], statFlag);
	}
      }
	...
	for (int j = 1; j<=binContent; j++){
      ...
      vector<float> xMax;
      //xMax = {0.2, 0.25, 0.4, 0.55, 0.6, 0.7, 1.5, 1.3};
      xMax = {0.15, 0.2, 0.4, 0.55, 0.6, 0.7, 0.9, 1.1};
      vector<double> xMax_PbWO4 = {0.2, 0.3, 0.6, 0.8, 1.1, 1.2, 1.6, 1.9};
      

      if (Material == "PbWO4"){
	rebin[0] = 5;
	(xmax[0] = xMax_PbWO4[EnergyIdx]);
      }
      
      else {xmax[0] = xMax[EnergyIdx]; rebin[0] = 1;}

      //cout << Material << ", " << Energy << ", " << xmax[0] << ", "<< xmin[0] << endl;
      cout << "xmax given is: " << xmax[0];
