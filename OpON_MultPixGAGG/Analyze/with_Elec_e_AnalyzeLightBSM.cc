  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
	...
	for (int iHit=0 ; iHit< 1; iHit++){	 
	  ...
	  if(nOp_crys != 0) {	    
	    ...
	    cout << "scattered e- Energy: "<<E_scatElec << endl;
	    cout << "total Edep cryst: "<<EDep_crys << endl;
	    cout << "Op Pho at end: "<< nOp_crys << "\n\n";
	...
	//if(Hit_Edep_truth[0]+Hit_Edep_truth[1] == 511){
	...
	if (Hit_Edep_truth[0] < Hit_Edep_truth[1]) theta_truth = GetScatAngle(Hit_Edep_truth[0]);
	else if (Hit_Edep_truth[1] < Hit_Edep_truth[0]) theta_truth = GetScatAngle(Hit_Edep_truth[1]);
