...
void AnalyzeLightBSM::EventLoop(const char *detType,const char *inputFileList, const char *outFileName) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
	...
	for (int iHit=0 ; iHit < 1; iHit++){     //considering only the first hit
	  ...
	  bool hit2_S1S1 =false, hit2_S1S2 =false, hit2_SB =false, hit2_B1B1 =false, hit2_B1B2 =false, hit2_BS =false, rest =false, hit2_AnyDiffDet = false;
	  bool teSt = false;	 	  
	  ...
	  if (procId !=0 && procId != 1){       //excluding rayleigh && phot (not required exactly but kept of safety purpose)
	    ...
	    if (hit2_AnyDiffDet) FillHistogram(nHit2_S_AnyDiffDet, Time, PosX, PosY, PosZ, scat_theta_SD_deg, EtaPolScat_deg, Pol0, Pol1, Pol2, Ein, Eout);
